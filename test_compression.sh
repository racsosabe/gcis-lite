...
if [ "$GROUP_MODE" = "-clean" ]; then
    echo "Limpiando carpetas '$OUTPUT_DIR' y '$LOG_DIR'..."
    rm -rf "$OUTPUT_DIR"
    rm -rf "$LOG_DIR"
...
LOG_FILE="${LOG_DIR}/benchmark_${GROUP_MODE}_${MODE}_${TIMESTAMP}.log"
...
for GROUP in "${GROUPS[@]}"; do
    ...
    GROUP_PATH="${INPUT_DIR}/${GROUP}/inputs/${GROUP}"

    if [ ! -d "$GROUP_PATH" ]; then
        echo "Grupo no encontrado: $GROUP_PATH"
        continue
    fi
    ...
    find "$GROUP_PATH" -type f | while read -r input_file; do

        base_name=$(basename "$input_file")

        for ENC in "${ENCODERS[@]}"; do

            output_file="${OUTPUT_DIR}/${GROUP}/${base_name}${ENC}.out"
            ...
            measure ./build/gcis_lite -c "$input_file" "$output_file" "$ENC" >> "$LOG_FILE" 2>&1
            ...
            if [ -f ../GCIS/build/src/gcis ]; then
              ...
              measure ../GCIS/build/src/gcis -c "$input_file" "$output_file" "$ENC" >> "$LOG_FILE" 2>&1
            else
              echo "Not comparing gcis_lite and GCIS, only generating gcis_lite performance"
            fi
...
echo "" | tee -a "$LOG_FILE"
echo "Benchmark finalizado: $(date)" | tee -a "$LOG_FILE"
